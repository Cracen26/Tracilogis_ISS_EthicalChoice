...
//				System.out.println("There is no such belief!!");
    ...
    // method to adapt mcapl 2025
    private boolean containsPercept(String agName, Predicate p) {
        Set<Predicate> ps = getPercepts(agName, false);
        if (ps == null) return false;

        for (Predicate per : ps) {
            if (per.unifies(p, new Unifier())) {
                return true;
            }
        }
        return false;
    ...
    private void consultPercept(String agName) {
        getPercepts(agName, true);
    }

    private VerifySet<Predicate> getEthicalPercepts(String agName) {
        VerifySet<Predicate> result = new VerifySet<>();
        Set<Predicate> ps = getPercepts(agName, false);
        if (ps == null) return result;

        for (Predicate p : ps) {
            if (p.getFunctor().equals("e")) {
                result.add(p);
            }
        }
        return result;
    ...
    private Predicate getPos(String agName) {
        Set<Predicate> ps = getPercepts(agName, false);
    ...
    private void removePos(String agName) {
        ...
        if (toRemove != null) {
            removePercept(agName, toRemove);
        }
    }

    private Predicate getPredicateTrace(String agName) {
        Set<Predicate> ps = getPercepts(agName, false);
        if (ps == null) return null;

        for (Predicate p : ps) {
            if (p.getFunctor().equals("Trace")) {
                return p;
            }
        }
        return null;
